  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-slate-400 text-sm">Resumo do dia e histórico de fechamentos.</p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
          ...
          {breakdownList.length === 0 ? (
            <p className="text-slate-500 text-sm">Nenhuma venda registrada ainda hoje.</p>
          ) : (
            <div className="space-y-3">
              {breakdownList.map((item) => (
                <div key={item.name} className="flex justify-between items-center p-3 bg-slate-950 rounded-lg border border-slate-800">
                  <div>
                    <span className="text-sm text-white font-medium">{item.name}</span>
                    <span className="text-xs text-slate-500 block">{item.qty} unidade(s)</span>
                  </div>
                  <span className="text-green-400 font-semibold text-sm">R$ {item.total.toFixed(2)}</span>
                </div>
              ))}
            </div>
          )}
          ...
          <button
            ...
            className="w-full mt-6 bg-rose-600 hover:bg-rose-500 disabled:bg-slate-700 disabled:cursor-not-allowed text-white font-medium py-2.5 rounded-lg transition-all flex items-center justify-center gap-2"
          >
            <Lock className="w-4 h-4" /> Fechar Caixa do Dia
          </button>

          {message && (
            <p className={`text-xs mt-3 ${message.type === 'error' ? 'text-rose-400' : 'text-green-400'}`}>
              {message.text}
            </p>
          )}
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
          ...
          {closingHistory.length === 0 ? (
            <p className="text-slate-500 text-sm">Nenhum fechamento realizado ainda.</p>
          ) : (
            <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
              {closingHistory.map((c) => (
                <div key={c.id} className="p-4 bg-slate-950 rounded-lg border border-slate-800">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-sm font-medium text-white">{c.date} às {c.closedAt}</span>
                    <span className="text-green-400 font-semibold text-sm">R$ {c.totalValue.toFixed(2)}</span>
                  </div>
